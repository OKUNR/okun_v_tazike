...
def show_note():
    ...
    print(key)
    field_text.setText(notes[key]['тєкст'])
    list_tags.clear()
...
def save_note():
    if list_notes.selectedItems():
        key = list_notes.selectedItems()[0].text()
        notes[key]['тєкст'] = field_text.toPlainText()
        with open("notes_data.json", "w", encoding='utf-8') \
                as file:
            json.dump(notes, file, sort_keys = True,
                      ensure_ascii=False)
        print(notes)
    else:
        print("Замєтка для зберєжения не вибрана!")
